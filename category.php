...
<html lang="en">
    <head>
        ...
        <!-- Bootstrap CSS -->
        <base href="http://localhost/RingtoneProject/cat">
        ...
        <style>
            ...
            @media (max-width: 767.98px) {
              ...
              <?php
                        include 'db.php';
                        include 'service.php';
                        $filter_query = $_GET['cat'];                       
                        if(($filter_query==NULL)){
                            header("Location:index.php");
                        } 
                        
                        $charFilter = str_replace("-", " ", $filter_query);
                        //Query for date filter
                        $Filtersql = "SELECT * FROM `data` WHERE live=1 and category like '%".$charFilter."%'";                                                
                        $FilterResult = mysqli_query($conn, $Filtersql);                        
                        $NoOfRow = mysqli_num_rows($FilterResult);
                ?>
        
        <div class="main" style="margin-top: 10px;"><br>
            <h4 class="display-5 jumbotron shadow-sm p-3 mb-5 bg-light border" style="padding:10px; text-align: center;"><?php echo strtoupper($charFilter);?></h4>            
